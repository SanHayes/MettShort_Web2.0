            ...
            var materialIndex = new Vue({
                ...
                methods: {
                        ...
                        switch (opttype) {
                            case 'delete':
                                that.$confirm('此操作将删除菜单, 是否继续?', '提示', {
                                    ...
                                    Fast.api.ajax({
                                        url: 'dramas/wechat_material/del/ids/' + id,
                                        loading: true,
                                        type: 'POST',
                                    }, function (ret, res) {
                                        that.getlistData()
                                ...
                                break;
                            case 'create':
                                Fast.api.open(`dramas/wechat_material/add?type=${that.activeName}`, "新建", {
                                    callback(data) {
                                        that.getlistData();
                                    }
                                });
                                break;
                            case 'edit':
                                Fast.api.open(`dramas/wechat_material/edit?id=${id}&type=${that.activeName}`, "编辑", {
                                    callback(data) {
                                        that.getlistData();
                                    }
                                });
                    ...
                    getlistData() {
                        var that = this;
                        Fast.api.ajax({
                            url: 'dramas/wechat_material/index',
                            loading: true,
                            type: 'GET',
                            data: {
                                offset: that.offset,
                        ...
                        if (type == 'yes') {
                            this.$refs[issub].validate((valid) => {
                                if (valid) {
                                    ...
                                    if (this.optType != 'add') {
                                        Fast.api.ajax({
                                            url: 'dramas/wechat_material/edit?id=' + that.text_id,
                                            loading: true,
                                            type: "POST",
                                            data: {
                                        ...
                                        Fast.api.ajax({
                                            url: 'dramas/wechat_material/add',
                                            loading: true,
                                            type: "POST",
                                            data: {
                                                data: JSON.stringify(subData)
