                ...
                methods: {
                    getData(id) {
                        ...
                        Fast.api.ajax({
                            ...
                            that.timeData = []
                            res.data.forEach(i => {
                                if(that.lang_list[i.lang_id] != undefined){
                            ...
                            if (that.tabsData.length > 0) {
                                if (id == null) {
                                    ...
                                    that.langId = that.tabsData[0].lang_id;
                                    that.activeIndex = 0;
                    ...
                    operation(type, id) {
                        ...
                        switch (type) {
                            case 'addCategory':
                                Fast.api.open("dramas/category/add", __('Add'), {
                                ...
                                break;
                            case 'editCategory':
                                Fast.api.open("dramas/category/edit?ids=" + id, __('Edit'), {
                                    callback: function (data) {
                                            if (data.type == 'edit') {
                                                that.getData(data.id)
                                            } else {
                            ...
                            case 'add':
                                if(that.activeIndex!=null){
                                    ...
                                    that.$notify({
                                        title: __('Warning'),
                                        message: __('Please add or select the main category first'),
                                        type: 'warning'
                                    });
                                }
                                break;
                            ...
                            case 'update':
                                ...
                                if (!isName) {
                                    that.$notify({
                                        title: __('Warning'),
                                        message: __('Classification name not filled in, cannot be saved and updated'),
                                        type: 'warning'
                                    });
                                    return false;
                                }
                                ...
                                if (subData.length > 0) {
                                    ...
                                    this.$notify({
                                        title: __('Warning'),
                                        message: __('There is no data to submit'),
                                        type: 'warning'
                                    });
                                    return false;
                                }
                                ...
                                that.isAjax = false;
                                Fast.api.ajax({
                                    url: 'dramas/category/update?ids=' + that.tabsData[that.activeIndex].id,
                                    loading: false,
                                    type: 'POST',
                                    data: {
                    ...
                    handleClick(tab) {
                        ...
                        that.langId = that.tabsData[index].lang_id;
                    ...
                    remove(node, data) {
                        if (data.children && data.children.length > 0) {
                            this.$notify({
                                title: __('Warning'),
                                message: __('Please delete the subcategory data first'),
                                type: 'warning'
                            });
                            return false;
                        } else {
            ...
            var categorySelect = new Vue({
                ...
                methods: {
                    ...
                    getData() {
                        let that = this;
                        Fast.api.ajax({
                            url: 'dramas/category/select?type='+this.type,
                            type: "GET",
                        }, function (ret, res) {
            ...
            var categoryDetail = new Vue({
                ...
                data() {
                    return {
                        ...
                        rulesForm: {
                            lang_id: [{
                                required: true,
                                message: __('Please select a classification language'),
                                trigger: 'blur'
                            }],
                            type: [{
                                required: true,
                                message: __('Please select a classification type'),
                                trigger: 'blur'
                            }],
                            name: [{
                                required: true,
                                message: __('Please enter the category name'),
                                trigger: 'blur'
                            }],
                            weigh: [{
                                required: true,
                                message: __('Please enter classification weight'),
                                trigger: 'blur'
                            }],
                            // status: [{
                            //     required: true,
                ...
                mounted() {
                    ...
                    if (this.optType == 'edit') {
                        ...
                        this.detailForm['lang_id'] = this.detailForm['lang_id'].toString();
                    }
                },
                methods: {
                    selectType(type) {
                    ...
                    deletecategory() {
                        let that = this;
                        that.$confirm(__('This operation will delete all relevant data for this category. Do you want to continue?'), __('Warning'), {
                            confirmButtonText: __('Ok'),
                            cancelButtonText: __('Cancel'),
                            type: 'warning'
                        }).then(() => {
                            Fast.api.ajax({
                                url: 'dramas/category/del/ids/' + Config.row.id,
                                loading: true,
                                type: 'POST'
                            }, function (ret, res) {
                                Fast.api.close({
                    ...
                    submitForm(check) {
                        ...
                        this.$refs[check].validate((valid) => {
                            if (valid) {
                                if (that.optType != 'add') {
                                    Fast.api.ajax({
                                        url: 'dramas/category/edit?ids=' + Config.row.id,
                                        loading: true,
                                        type: "POST",
                                        data: {
