...
class Video extends Model
{
    ...
    public function getCategoryList($site_id, $lang_id, $is_column=false)
    {
        ...
        $category_list = [];
        if($category){
            $model_cat = Category::where('pid', $category['id'])
                ->where('type', 'video')
                ->orderRaw('weigh desc, id asc');
            if($is_column){
                $category_list = $model_cat->column('name', 'id');
            }else{
                $category_list = $model_cat->select();
            }
        }
    ...
    public function getCategoryTextAttr($value, $data)
    {
        ...
        $site_id = Auth::instance()->id;
        $list = $this->getCategoryList($site_id, $data['lang_id'], true);
    ...
    public function getCopyrightIdsListAttr($value, $data)
    {
        $copyright_ids_list = [];
        if(check_env('copyright', false) == true && isset($data['copyright_id'])){
            $copyright_ids_list[] = \app\admin\model\dramas\Copyright::where('id', $data['copyright_id'])->field('id, name')->find();
        }
        return $copyright_ids_list;
    }
