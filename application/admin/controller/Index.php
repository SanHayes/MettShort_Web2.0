...
class Index extends Backend
{
    ...
    public function login()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if($admin){
                ...
                if($site_info && $site_info['domain'] && $site_info['domain'] != $_SERVER['HTTP_HOST']){
                    $this->error(__('Please log in using the bound domain name: %s', $site_info['domain']), $url, ['token' => $this->request->token()]);
                }
            }
        ...
        $config = \addons\dramas\model\Config::where('site_id', $site_id)->where('name', 'dramas')->find();
        $config = $config ? json_decode($config['value'], true) : [];
        $lang = $this->request->langset();
        $lang = preg_match("/^([a-zA-Z\-_]{2,10})\$/i", $lang) ? $lang : 'zh-cn';
        $title = $config['name'][$lang] ?? 'Dramas';
