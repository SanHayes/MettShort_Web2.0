...
class Sites extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $row) {
                ...
                $row->console = 1;
                $data = Http::post('http://console.nymaite.cn/api/index/check', ['type'=>'dramas', 'site'=>$domain]);
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if($params['is_default'] == 1 && isset($params['domain']) && $params['domain'] != $_SERVER['HTTP_HOST']){
