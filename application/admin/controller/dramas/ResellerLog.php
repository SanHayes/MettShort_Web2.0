...
class ResellerLog extends Backend
{
    ...
    /**
     * ResellerLog模型对象
     * @var \app\admin\model\dramas\ResellerLog
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\dramas\ResellerLog;
        $this->assignconfig("typeList", $this->model->getTypeList());
        $this->assignconfig("orderTypeList", $this->model->getOrderTypeList());
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $nobuildfields = ['lang_id', 'nickname', 'user_email'];
            ...
            $count['total_logs'] = $this->buildSearchOrder()
                ->where($where)
                ->count();
            $count['total_pay_money'] = $this->buildSearchOrder()
                ->where($where)
                ->sum('pay_money');
            $count['total_money'] = $this->buildSearchOrder()
                ->where($where)
                ->sum('money');
            $count['withdraw_total_money'] = $this->buildSearchOrder()
                ->where($where)
                ->sum('total_money');
    ...
    private function buildSearchOrder()
    {
        ...
        if ($lang_id) {
            $orders = $orders->whereExists(function ($query) use ($lang_id, $tableName) {
                $videoTableName = (new \app\admin\model\dramas\Video())->getQuery()->getTable();
