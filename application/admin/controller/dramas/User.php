...
class User extends Backend
{
    ...
    protected $noNeedRight = ['lists', 'changeParentUser', 'changeVip', 'changeReseller'];
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
                ...
                ->where(function ($query) use ($searchWhere) {
                    if ($searchWhere) {
                        ...
                        $query = $query->table($table_name)->whereOr($table_name.'.id', '=', $searchWhere)
                            ...
                            ->whereOr($table_name.'.email', 'like', "%{$searchWhere}%")
            ...
            foreach ($list as $k => $v) {
                ...
                $v->third_platform = UserOauth::all(['user_id' => $v->id, 'provider'=>['<>', 'Guest']]);
            }
            $result = array("total" => $total, "rows" => $list);

            $this->success(__('View'), null, $result);
        }
        $this->assignconfig('groupList', UserGroup::field('id,name,status')->select());
    ...
    public function lists(){
        ...
        if (isset($user_ids) && $user_ids !== '') {
            $order = 'field(id, ' . $user_ids . ')';       // 如果传了 user_ids 就按照里面的 id 进行排序
    ...
    public function profile($id)
    {
        ...
        $row->third_platform = UserOauth::all(['user_id' => $row->id, 'provider'=>['<>', 'Guest']]);
    ...
    public function select()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $mode = $this->request->request('mode', 'user');
            $modeWhere = [];
            if($mode != 'all'){
                $modeWhere['mode'] = $mode;
            }
            $modeWhere['status'] = 'normal';
            $total = $this->model
                ->where($modeWhere)
                ->where(function ($query) use ($where, $searchWhere){
                    if($query){
                        $query->where($where);
                    }
                    if($searchWhere){
                        $query->whereOr('id', '=', $searchWhere)
                            ...
                            ->whereOr('email', 'like', "%$searchWhere%");
                    }
                })
                ->order($sort, $order)
                ->count();
            ...
            $list = $this->model
                ...
                ->order($sort, $order)
                ->field('id, nickname, mobile, email, avatar, usable, mode')
                ->limit($offset, $limit)
                ->select();
            $result = array("total" => $total, "rows" => $list);
    ...
    public function changeParentUser($id)
    {
        ...
        try {
            ...
            if($user['mode'] == 'user'){
                ...
                $lang_id = Lang::where('lang', $lang)->value('id');
                $share_id = $share['share_id'];
                $data = ['user_id'=>$share_id, 'site_id'=>$user['site_id'], 'lang_id'=>$lang_id];
