...
use think\Db;
use addons\dramas\model\UserWalletApply as WithDraw;
...
class UserWalletApply extends Backend
{
    ...
    /**
     * UserWalletApply模型对象
     * @var \app\admin\model\dramas\UserWalletApply
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\dramas\UserWalletApply;
    ...
    public function getType()
    {
        ...
        return $this->success(__('Operation completed'), null, $data);
    }
            ...
            try {
                switch ($operate) {
                    ...
                    case '-1':
                        ...
                        if (!$rejectInfo) {
                            throw new \Exception(__('Please enter the reason for rejection'));
                        }
                ...
                Db::rollback();
                WithDraw::handleLog($apply, __('Operation failed').': ' . $e->getMessage());
                $failedCount++;
                $lastErrorMessage = $e->getMessage();
            }
        }
        if (count($ids) === 1) {
            if ($successCount) $this->success(__('Operation completed'));
            if ($failedCount) $this->error($lastErrorMessage);
        } else {
            $this->success(__('Success: %d | Failure: %d', $successCount, $failedCount));
        }
    }
    ...
    public function log($id)
    {
        ...
        if ($applyLog) {
            foreach ($applyLog as &$log) {
                $log['oper'] = \addons\dramas\library\Oper::get($log['oper_type'], $log['oper_id']);
            }
        }
        $this->success('log', null, $applyLog);
    }
