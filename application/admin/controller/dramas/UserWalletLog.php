...
class UserWalletLog extends Backend
{
    ...
    /**
     * UserWalletLog模型对象
     * @var \app\admin\model\dramas\UserWalletLog
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\dramas\UserWalletLog;
        $this->assignconfig("walletTypeList", $this->model->getWalletTypeList());
        $this->assignconfig("operTypeList", $this->model->getOperTypeList());
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            list($where, $sort, $order, $offset, $limit) = $this->custombuildparams();
            ...
            $count['surplus_coins'] = $this->model
                ->where($where)
                ->sum('wallet');
            $count['add_coins'] = $this->model
                ->where($where)
                ->where('wallet', '>=', 0)
                ->sum('wallet');
            $count['consumption_coins'] = $this->model
                ->where($where)
                ->where('wallet', '<', 0)
                ->sum('wallet');
