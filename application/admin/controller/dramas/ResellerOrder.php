...
class ResellerOrder extends Backend
{
    ...
    /**
     * ResellerOrder模型对象
     * @var \app\admin\model\dramas\ResellerOrder
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\dramas\ResellerOrder();
    ...
    public function detail($id)
    {
        if ($this->request->isAjax()) {
            ...
            $row = $this->model->withTrashed()->with(['user', 'reseller'])->where($table.'.id', $id)->find();
            if (!$row) {
                $this->error(__('No Results were found'));
            }

            return $this->success('', null,  $row);
        }
    ...
    private function buildSearchOrder($is_status=true)
    {
        ...
        if ($lang_id) {
            $orders = $orders->whereExists(function ($query) use ($lang_id, $tableName) {
                $resellerTableName = (new \app\admin\model\dramas\Reseller())->getQuery()->getTable();

                $query = $query->table($resellerTableName)->where($resellerTableName . '.id=' . $tableName . 'reseller_id');
