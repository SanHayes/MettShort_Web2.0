...
class VipOrder extends Backend
{
    ...
    /**
     * VipOrder模型对象
     * @var \app\admin\model\dramas\VipOrder
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\dramas\VipOrder;
    ...
    public function detail($id)
    {
        if ($this->request->isAjax()) {
            ...
            $row = $this->model->withTrashed()->with(['user', 'vip'])->where($table.'.id', $id)->find();
            if (!$row) {
    ...
    private function buildSearchOrder($is_status=true)
    {
        ...
        if ($lang_id) {
            $orders = $orders->whereExists(function ($query) use ($lang_id, $tableName) {
                $vipTableName = (new \app\admin\model\dramas\Vip())->getQuery()->getTable();

                $query = $query->table($vipTableName)->where($vipTableName . '.id=' . $tableName . 'vip_id');
