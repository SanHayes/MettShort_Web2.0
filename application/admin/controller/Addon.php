    ...
    public function downloaded()
    {
        ...
        foreach ($addons as $k => $v) {
            ...
            if (isset($onlineaddons[$v['name']])) {
                ...
                // TODO 本地更新
                $version = Http::post('http://console.nymaite.cn/api/index/version', ['site'=>$_SERVER['HTTP_HOST'], 'version'=>$v['version'], 'type'=>$v['name'], 'is_update'=>false]);
                $version = @json_decode($version, true);
                if(isset($version['data']['newversion'])){
                    $v['version'] = $version['data']['newversion'];
                }
                $v['category_id'] = 0;
                $v['flag'] = '';
                $v['banner'] = '';
