...
class Menu
{
    ...
    public static function upgrade($name, $menu)
    {
        ...
        try {
            ...
            if(Menu::check_env('copyright', false) == true){
                $rule_ids = Db::name('auth_rule')->whereLike('name', '%copyright%')->column('id');
                $ids = array_diff($ids, $rule_ids);
            }
            if(Menu::check_env('copyright', false) == false){
                $rule_ids = Db::name('auth_rule')->whereLike('name', '%copyright%')->column('id');
                $ids = array_unique(array_merge($ids, $rule_ids));
            }
            if ($ids) {
                //旧版本的菜单需要做删除处理
    ...
    public static function check_env($need = [], $is_throw = true)
    {
    ...
    public static function getAuthRuleIdsByName($name)
    {
        ...
        if($name == 'dramas'){
            $yingxiao = AuthRule::getByName('dramas/yingxiao');
            if($yingxiao){
