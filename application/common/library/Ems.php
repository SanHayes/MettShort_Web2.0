        ...
        if (!Hook::get('ems_send')) {
            ...
            Hook::add('ems_send', function ($params) use ($site_id, $lang) {
                ...
                if($msg){
                    ...
                    $dramasConfig = \addons\dramas\model\Config::where('site_id', $site_id)->where('name', 'dramas')->value('value');
                    $dramasConfig = @json_decode($dramasConfig, true);
                    if($dramasConfig){
                        $copyright = '';
                        $copyright_list = $dramasConfig['copyright']['list'] ?? [];
                        foreach ($copyright_list as $item){
                            $copyright .= $item['name'].' ';
                        }
                        $copyright = trim($copyright);
                        $title = $dramasConfig['name'][$lang] ?? '';
                        $domain = $dramasConfig['h5'] ?? '';
                        $logo = $dramasConfig['company'] ? cdnurl($dramasConfig['company'], true) : '';
                        $msg = str_replace('{code}', $params->code, $msg);
                ...
                $result = $obj
                    ->to($params->email)
                    ->subject(__('%s is your verification code!', $params->code))
                    ->message($msg)
                    ->send();
                if($result == false){
