...
use addons\dramas\model\Notification;
...
{
    protected $noNeedLogin = ['init', 'about_us', 'lang_list', 'lang_data', 'index', 'notification', 'richtext', 'check_host', 'version'];
    ...
    public function index()
    {
        ...
        // 公共方法
        require_once ADDON_PATH . 'dramas/helper.php';
        if(check_env('copyright', false) == false){
            $ids = Db::name('auth_rule')->whereLike('name', '%copyright%')->column('id');
            $ruleIdList2 = array_diff($ruleIdList2, $ids);
        }
    ...
    public function init()
    {
        ...
        $configFields = ['dramas', 'share', 'user', 'google', 'facebook'];    // 定义设置字段
        ...
        $dramasConfig['h5'] = request()->domain().'/h5?sign='.$this->sign.'#/';
        ...
        if(isset($config['google'])){
            $platformConfig = json_decode($config['google'], true);
            $data['google'] = ['appid' => isset($platformConfig['app_id']) ? $platformConfig['app_id'] : ''];
        }
        $data['facebook'] = null;
        if(isset($config['facebook'])){
            $platformConfig = json_decode($config['facebook'], true);
            $data['facebook'] = ['appid' => isset($platformConfig['app_id']) ? $platformConfig['app_id'] : ''];
        }
        // 会员设置
    ...
    public function notification(){
        ...
        $list = Notification::where('platform', $platform)
