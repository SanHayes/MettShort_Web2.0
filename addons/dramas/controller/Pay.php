...
class Pay extends Base
{
    ...
    protected $noNeedGuest = ['alipay', 'paypal', 'prepay', 'checkPay', 'walletPay'];
    ...
    public function paypal(){
        ...
        if($payment == 'paypal'){
            ...
            $cancel_url = $cancel_url . (strpos($cancel_url, '?') === false ? '?' : '');
            $postData = [
            ...
            $postData['returnUrl'] = $this->request->root(true) . '/h5?sign=' . $this->sign . '#' . $return_url;
            $postData['cancelUrl'] = $this->request->root(true) . '/h5?sign=' . $this->sign . '#' . $cancel_url;
            ...
            if($platform == 'App'){
                ...
                $return_url = $return_url . (strpos($return_url, '?') === false ? '?session_id={CHECKOUT_SESSION_ID}' : '&session_id={CHECKOUT_SESSION_ID}');
                $cancel_url = $cancel_url . (strpos($cancel_url, '?') === false ? '?' : '');
                ...
                try {
                    $session = \Stripe\Checkout\Session::create([
                        ...
                        'metadata' => ['order_sn' => $order->order_sn],
                        'mode' => 'payment',
                        'success_url' => $this->request->root(true) . '/h5?sign=' . $this->sign . '#' . $return_url,
                        'cancel_url' => $this->request->root(true) . '/h5?sign=' . $this->sign . '#' . $cancel_url,
                    ]);
                }catch (\Exception $e){
                    $this->error(__('Payment error:%s', [$e->getMessage()]));
                }
