...
use addons\dramas\library\Wechat as WechatLibrary;
use addons\dramas\model\Config;
use addons\dramas\model\Wechat as WechatModel;
    ...
    private function response($replyInfo, $event='text')
    {
        switch ($event) {
            case 'subscribe':
                ...
                $sign = request()->param('sign');
                $url = urlencode(request()->domain().'/addons/dramas/user/wxOfficialAccountLogin?key='.$key.'&sign='.$sign);
                $config = Config::getEasyWechatConfig('wxOfficialAccount');
                ...
                $system = $configModel->where('site_id', $this->site_id)->where('name', 'dramas')->value('value');
                // 基本设置
