...
class PayService
{
    ...
    {
        $paymentConfig = @json_decode(\addons\dramas\model\Config::get(['name' => $this->payment, 'site_id'=>$this->site_id])->value, true);

        if(empty($paymentConfig)){
            throw new \Exception('Please configure payment first');
        }

        // 如果是支付，并且不是 复制地址的支付宝支付
        if ($this->type == 'pay' && $this->platform != 'url' && !in_array($this->platform, $paymentConfig['platform'])) {
            throw new \Exception('This payment method is currently not supported');
        }
    ...
    private function setWechatAppId()
    {
        switch ($this->platform) {
            case 'wxOfficialAccount':
                $platformConfig = json_decode(\addons\dramas\model\Config::get(['name' => $this->platform, 'site_id'=>$this->site_id])->value, true);
            ...
            case 'Web':
                ...
                break;
            case 'App':
                $platformConfig = json_decode(\addons\dramas\model\Config::get(['name' => 'App', 'site_id'=>$this->site_id])->value, true);
                if (isset($this->config['mode']) && $this->config['mode'] === 'service') {
        ...
        switch ($this->payment) {
            ...
            case 'alipay':
                if (in_array($this->platform, ['wxOfficialAccount', 'wxMiniProgram', 'H5'])) {
                    // 返回支付宝支付链接
                    $pay = request()->domain() . '/addons/dramas/pay/alipay?order_sn=' . $order['out_trade_no'];
                } else {
                    if ($this->method == 'wap') {
                        // 支付宝 wap 支付，增加 return_url
                        // 获取 h5 域名
                        $platformConfig = json_decode(\addons\dramas\model\Config::get(['name' => 'dramas', 'site_id'=>$this->site_id])->value, true);
