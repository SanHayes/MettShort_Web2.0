...
use addons\dramas\model\UserSign;
use think\Db;
...
class UserSignService
{
    ...
    public function signin()
    {
        $signin = Db::transaction(function () {
            ...
            if ($score > 0) {
                ...
                User::usable($score, $user, 'usersign', $signin['id'], 'Sign in and receive points', [
                    'date' => date('Y-m-d', $totime),
    ...
    public function replenish($params)
    {
        $signin = Db::transaction(function () use ($params) {
            if (!$this->rules['is_replenish']) {
                $this->error_stop(__('Current check-in does not allow supplementary signing'));
            }
            ...
            $signin = UserSign::where('user_id', $this->user->id)
                ...
                ->lock(true)->find();
            if ($signin) {
                $this->error_stop(__('You have already checked in on that day, there is no need to add a new check-in'));
            }
            ...
            if ($this->rules['replenish_num'] > 0) {
                $user = User::get($this->user->id);
                User::usable(-$this->rules['replenish_num'], $user, 'replenish_usersign', $signin['id'], 'Supplementary signature deduction', [
                    'date' => $params['date'],
                    'user_sign_id' => $signin['id']
                ]);
            }
    ...
    private function isReplenish($date, $is_throw = true)
    {
        ...
        if ($interval_days <= 0) {
            return $this->exception(__('Only dates before today can be re signed'), $is_throw);
        }
        if ($this->rules['replenish_limit'] && $interval_days > $this->rules['replenish_limit']) {
            return $this->exception(__('The maximum supplementary signing date has been exceeded'), $is_throw);
        }
        ...
        if (($need_days - $real_days) >= ($this->rules['replenish_days'] - $replenish_days)) {
            return $this->exception(__('Up to the last %s days can be re signed, and the currently selected date cannot be re signed', $this->rules['replenish_days']), $is_throw);
        }
    ...
    {
        $user_sign = Config::get(['name' => 'usersign']);
        $user_sign = @json_decode($user_sign->value, true);
        if(empty($user_sign)){
            $this->error_stop(__('Please configure usersign'));
        }
