...
class Sms
{
    ...
    private function aliNotice($params)
    {
        ...
        $lang_ = $params['nation_code'] == '86' ? 'zh-cn' : 'en';
        $param = $param ? $param : [];
        $params['template'] = isset($params['template']) ? $params['template'] : (isset($lang_) && isset($this->config['template'][$lang_]) && $this->config['template'][$lang_] ? $this->config['template'][$lang_] : '');
        $result = $alisms->mobile($params['nation_code'].$params['mobile'])
    ...
    private function qcloudSend($params)
    {
        try {
            if ($this->config['isTemplateSender'] == 1) {
                ...
                $sign = $lang_ == 'en' ? $this->config['sign_en'] : $this->config['sign_cn'] ;
                //普通短信发送
                $sender = new SmsSingleSender($this->config['appid'], $this->config['appkey']);
                $result = $sender->sendWithParam($params['nation_code'], $params['mobile'], $templateID, ["{$params->code}"], $sign, "", "");
            } else {
                $sender = new SmsSingleSender($this->config['appid'], $this->config['appkey']);
    ...
    private function qcloudNotice($params)
    {
        try {
            if ($this->config['isTemplateSender'] == 1) {
                ...
                //普通短信发送
                $sender = new SmsSingleSender($this->config['appid'], $this->config['appkey']);
                $result = $sender->sendWithParam($params['nation_code'], $params['mobile'], $templateID, ["{$params['msg']}"], $sign, "", "");
            } else {
                $sender = new SmsSingleSender($this->config['appid'], $this->config['appkey']);
                //参数：短信类型{1营销短信，0普通短信 }、国家码、手机号、短信内容、扩展码（可留空）、服务的原样返回的参数
                $result = $sender->send($params['type'], $params['nation_code'], $params['mobile'], $params['msg'], "", "");
