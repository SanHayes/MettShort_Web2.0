...
class VideoLog extends Model
{
    ...
    // 表名
    protected $name = 'dramas_video_log';
    ...
    public static function edit($params)
    {
        ...
        return Video::where('id', $vid)->field(Video::$fields)->find();
    }
            ...
            foreach ($ids as $g) {
                ...
                if($log){
                    ...
                    if($type == 'favorite'){
                        ...
                        $video = Video::where('id', $log->vid)->field(Video::$fields)->find();
                    }
                }
            }
        }
    ...
    public static function getVideosList($type, $page, $pagesize)
    {
        ...
        $favoriteData = self::with(['video' => function ($query) {
            $query->removeOption('soft_delete')->field(Video::$fields);
        }, 'episode' => function ($query) {
            $query->removeOption('soft_delete')->field(VideoEpisodes::$fields);
        }])->where([
            'user_id' => $user->id,
            'type' => $type
