...
class Category extends Model
{
    ...
    // 表名,不含前缀
    protected $name = 'dramas_category';
    // 自动写入时间戳字段
    ...
    public static function getCategoryList($id, $site_id, $lang_id)
    {
        $category = self::where(['site_id' => $site_id, 'lang_id' => $lang_id, 'pid' => $id, 'status' => 'normal'])->with('children.children.children')->select();
    ...
    {
        return $this->hasMany(\addons\dramas\model\Category::class, 'pid', 'id')->where('status', 'normal')->order('weigh desc, id asc');
    }
