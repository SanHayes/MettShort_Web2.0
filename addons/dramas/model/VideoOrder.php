...
class VideoOrder extends Model
{
    ...
    public static function addVideoEpisodes($platform, $episodes){
        ...
        $order_id = Db::transaction(function () use ($user, $vid, $episode_id, $price, $order_sn, $platform){
            ...
            User::usable(-$price, $user, 'used_video', $video_order->id, 'Pay for watching dramass', [
                'request_id'=>$video_order->id
            ]);
            return $video_order->id;
        });
        if(check_env('copyright', false)){
            try {
                $params['order_id'] = $order_id;
                \think\Db::transaction(function () use ($params) {
                    \think\Hook::listen('order_after', $params);
                });
            } catch (\Exception $e) {}
        }
        return true;
    ...
    public static function addVideos($platform, $video){
        ...
        $order_sn = self::getSn($user->id);
        $order_id = Db::transaction(function () use ($user, $id, $price, $order_sn, $platform){
            $user = User::where('id', $user->id)->lock(true)->find();
            $data = [
                'site_id' => $user->site_id,
                'vid' => $id,
            ...
            User::usable(-$price, $user, 'used_video', $video_order->id, 'Purchase the entire dramas', [
                'request_id'=>$video_order->id
            ]);
            return $video_order->id;
        });
